...
function setupGoalTypeHandler() {
  ...
  goalTypeSelect.addEventListener('change', (e) => {
    ...
    workoutTypeField.style.display = goalType === 'specific_type' ? 'block' : 'none';
...
function displayActiveGoals(activeGoals) {
  ...
  if (activeGoals.length === 0) {
    container.innerHTML = '<p class="no-goals">No active goals. Set a new goal to get started!</p>';
    return;
  }
  ...
  container.innerHTML = activeGoals.map(goal => {
    ...
    return `
      <div class="goal-card">
        ...
        ${motivationalMessage ? `<p class="motivational-message">${motivationalMessage}</p>` : ''}
      </div>
    `;
  }).join('');
}
...
function displayCompletedGoals(completedGoals) {
  ...
  if (completedGoals.length === 0) {
    container.innerHTML = '<p class="no-goals">No completed goals yet. Keep working!</p>';
    return;
  }
...
function formatGoalTitle(goal) {
  ...
  if (goal.type === 'specific_type') {
    baseTitle = `${goal.target} ${capitalizeFirstLetter(goal.workoutType)} workouts`;
  } else if (goal.type === 'category') {
    baseTitle = `${goal.target} ${capitalizeFirstLetter(goal.category)} workouts`;
  }
